/* lowvision.js — DualView UX Low Vision Demo */
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderArticle(cat) {
    ...
    var linkText = isAccessible ? ("Read: " + data.readLinkTitle) : data.readLinkBad;
    var showOverlay = cfg.alttext ? "visible" : "";

    var paragraphs = data.body.map(function (p) {
      return "<p>" + p + "</p>";
    }).join("");
    ...
    panel.innerHTML =
      ...
      '<p class="article-byline">' + data.byline + '</p>' +
      '<div class="article-hero-wrap">' +
        ...
        '<div class="alt-overlay ' + showOverlay + '" id="article-overlay">' +
          (showOverlay ? "Screen reader: " + altVal : "") +
        '</div>' +
      '</div>' +
      '<p class="article-img-caption">' + data.imgCaption + '</p>' +
      '<div class="article-body">' + paragraphs + '</div>' +
      '<p style="margin-top:1.25rem;">' +
        '<a href="#" class="article-read-link" id="article-link"' +
           ' aria-label="' + (isAccessible ? "Read: " + data.readLinkTitle : "Read full article") + '">' +
           linkText +
        '</a>' +
      '</p>';
  }

  /* ---- Hook: runs after every apply() ---- */
